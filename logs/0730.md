...
# Experiment 5結論
Persistence baseline 在 5、10、15 分鐘後的 CPU temperature prediction 中皆優於 XGBoost。此結果表示目前 24 小時資料中的 CPU temperature 變化相對平穩，因此直接使用目前溫度作為未來預測值即可得到良好結果。後續若要展現 XGBoost 或其他機器學習模型的優勢，應收集更長時間資料，或加入不同負載情境，使溫度、功率與風扇轉速產生更明顯變化。
